...
drop function if exists public.current_user_owns_event_participant(uuid);
drop function if exists public.current_user_can_manage_event_participant(uuid, public.account_role[]);
drop function if exists public.current_user_can_access_event_participant(uuid);
drop function if exists public.current_user_can_manage_poll(uuid, public.account_role[]);
drop function if exists public.current_user_can_access_poll(uuid);
drop function if exists public.current_user_can_manage_event(uuid, public.account_role[]);
drop function if exists public.current_user_can_access_event(uuid);
drop function if exists public.current_user_owns_membership(uuid);
drop function if exists public.current_user_has_account_role(uuid, public.account_role[]);
drop function if exists public.current_user_has_account_access(uuid);
...
create or replace function app_private.current_user_has_account_role(
...
create or replace function app_private.current_user_can_manage_event(
...
as $$
  select exists (
    ...
    from public.events event
    where event.id = target_event_id
      and app_private.current_user_has_account_role(event.account_id, allowed_roles)
  );
$$;
...
create or replace function app_private.current_user_can_manage_poll(
  target_poll_id uuid,
  allowed_roles public.account_role[]
)
returns boolean
...
as $$
  select exists (
    ...
    from public.event_polls poll
    where poll.id = target_poll_id
      and app_private.current_user_can_manage_event(poll.event_id, allowed_roles)
  );
$$;
...
create or replace function app_private.current_user_can_manage_event_participant(
...
as $$
  select exists (
    ...
    from public.event_participants participant
    where participant.id = target_event_participant_id
      and app_private.current_user_can_manage_event(participant.event_id, allowed_roles)
  );
$$;
...
using (
  profile_id = auth.uid()
  or app_private.current_user_has_account_role(
    account_id,
...
to authenticated
using (
  app_private.current_user_has_account_role(account_id, array['group_admin'::public.account_role])
)
with check (
  app_private.current_user_has_account_role(account_id, array['group_admin'::public.account_role])
);
...
to authenticated
using (
  app_private.current_user_can_manage_event(event_id, array['group_admin'::public.account_role])
)
with check (
  app_private.current_user_can_manage_event(event_id, array['group_admin'::public.account_role])
);
...
to authenticated
using (
  app_private.current_user_can_manage_event(
    event_id,
    array['group_admin'::public.account_role, 'group_moderator'::public.account_role]
  )
)
...
to authenticated
using (
  app_private.current_user_can_manage_poll(
    poll_id,
    array['group_admin'::public.account_role, 'group_moderator'::public.account_role]
  )
)
...
with check (
  ...
  or app_private.current_user_can_manage_poll(
    poll_id,
    array['group_admin'::public.account_role, 'group_moderator'::public.account_role]
  )
);
...
to authenticated
using (
  app_private.current_user_can_manage_event_participant(
